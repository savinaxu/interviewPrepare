...
function jumpingOnClouds(c) {
    const thunderheads = c.map((num, index) => num === 1 ? index : undefined).filter(x => x)

    if (thunderheads.length > 0) {
        let jumps = 0,
            preIndex = 0,
            nextIndex = 0
        
        
        for (let i = 0; i < thunderheads.length; i++) {
            preIndex = thunderheads[i] - 1
            if (i === 0) {
                jumps += Math.ceil(preIndex / 2) + 1
                nextIndex = thunderheads[i] + 1
            } else {
                jumps += Math.ceil((preIndex - nextIndex) / 2) + 1
                nextIndex = thunderheads[i] + 1
            }
        }

        if (nextIndex === c.length - 1) return jumps
        else {
            jumps += Math.ceil((c.length - 1 - nextIndex) / 2)
            return jumps
        }
...
console.log(jumpingOnClouds([0,1,0,0,0,0,1,0,0]))
